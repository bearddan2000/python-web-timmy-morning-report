...
def get_links():
    ...
    x = resp.json()
    res_lst =  seq(x['links'])\
        .filter(lambda val: "graph" in val)\
        .filter(lambda val: "/weekday/" in val)\
        .filter(lambda val: "/month/" not in val)\
        .filter(lambda val: "/quarter/" not in val)\
        .filter(lambda val: "/other/" in val)
