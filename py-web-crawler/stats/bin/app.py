...
def get_links():
    ...
    res_lst =  seq(x['links'])\
        ...
        .filter(lambda val: "/trends/" not in val)\
        .filter(lambda val: "/notes/" not in val)\
        .filter(lambda val: "/distribution/" not in val)\
        .filter(lambda val: "/datatable/" not in val)\
        .filter(lambda val: "/correlation/" not in val)\
        .filter(lambda val: "/self/" not in val)\
        .filter(lambda val: "/other/" not in val)
